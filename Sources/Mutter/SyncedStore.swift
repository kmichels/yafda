...
///
/// Runs on launch, quit, wake, app activation, a debounced window after a
/// local write, and an hourly backstop (AMUX-756; see `SyncScheduler` for
/// the triggers and `StoreOwner` for the serialization that makes running
/// this all day safe). The merge is order-independent so simultaneous use on
/// two Macs degrades to a conflict on individual entries rather than a lost
...
enum SyncedStore {
    ...
    ///
    /// The whole cycle runs inside `StoreOwner.syncCycle` so it can never
    /// interleave with a concurrent store mutation, and so the stores'
    /// `save()` calls it makes internally know to skip the after-write
    /// debounce trigger (`StoreOwner.isRunningSyncCycle`).
    static func syncAll() {
    ...
    /// Wrapped in its own `StoreOwner.syncCycle`, not just relying on the
    /// outer `syncAll`'s: self-tests (and this file's own composition tests)
    /// call this directly against temp directories, bypassing `syncAll`
    /// entirely, and still need the same serialization guarantee. Nesting a
    /// `syncCycle` inside `syncAll`'s is safe - see `StoreOwner`.
    ...
    /// Wrapped in its own `StoreOwner.syncCycle` - see `syncLearned`.
